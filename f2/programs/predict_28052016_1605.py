...
def build_sample(regressor, name):
	...
	for i in range(n):
		X_train, X_cv, y_train, y_cv = cross_validation.train_test_split(
		      sample_X, sample_y, test_size=.20, random_state=i*SEED)
		# train...
	...
	# predict
	predict_res = regressor.predict(sample_X)
	# mape
	mape_rr = mape(sample_y, predict_res)
	# print
	print "MAPE of fold %s : %f" % (name, mape_rr)
	# predict on test
	predict_res_on_tes = regressor.predict(sample_t)
	# FRame
	preds_on_test = DataFrame(list(zip(sample_id, predict_res_on_tes)), columns = ["ID", "COTIS"])
	# save predictions
...
sample_X = sample_mapper[0:300000, 1:32]
sample_y = sample_mapper[0:300000, 32]
sample_t = sample_mapper[300000:330000, 1:32]
...
predict_res_4 = build_sample(RandomForestRegressor(random_state = None, min_samples_leaf = 20, verbose=1 ), "RandomForestRegressor")
predict_res_5 = build_sample(XGBRegressor(objective = "reg:linear"), "XGBRegressor", verbose=1)
